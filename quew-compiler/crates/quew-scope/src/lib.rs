...
use quew_ast::{
    AnnotationArgs, FieldDef, FunctionDecl, Item, Module,
    Param, ParamBinding, Provider, ToolDecl, ToolEntry,
    TypeExpr,
};
use quew_errors::{Diagnostic, Severity, Span};
use quew_interner::InternedStr;
use quew_lexer::AnnotationKind;
use quew_types::{AgentTy, FunctionTy, ProviderKind, Ty, ToolTy};

// ── SymbolKind ────────────────────────────────────────────────────────────────

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SymbolKind {
    Let, Function, Agent, Tool, ToolGroup, Type, Model, Param, Local,
}
...
pub fn lower_type(expr: &TypeExpr, diags: &mut Vec<Diagnostic>) -> Ty {
    match expr {
        ...
        TypeExpr::Optional(inner, _) => {
            lower_type(inner, diags).optional()
        }
    ...
    for ann in &func.annotations {
        if ann.kind == AnnotationKind::Tool {
            if let AnnotationArgs::Params(params) = &ann.args {
                return params.iter().map(|p| (p.name, lower_param_ty(p, diags))).collect();
            }
        }
    }
    vec![]
    ...
    for item in &module.items {
        ...
        match item {
            ...
            Item::Agent(decl) => {
                ...
                let return_ty = decl.return_ty.as_ref()
                ...
                table.define_global(decl.name, Symbol {
                    ty, kind: SymbolKind::Agent, def_span: decl.span,
                });
            }
            ...
            Item::Function(decl) => {
                ...
                table.diagnostics.extend(d);
                table.define_global(decl.name, Symbol { ty, kind, def_span: decl.span });
            }
            ...
            Item::Tools(decl) => {
                ...
                if let Some(name) = decl.name {
                    table.diagnostics.extend(d);
                    table.define_global(name, Symbol {
                        ty: Ty::void(),
            ...
            Item::Type(decl) => {
                ...
                table.diagnostics.extend(d);
                table.define_global(decl.name, Symbol {
                    ty, kind: SymbolKind::Type, def_span: decl.span,
                });
            }
            ...
            Item::Model(decl) => {
                ...
                table.define_global(decl.name, Symbol {
                    ty, kind: SymbolKind::Model, def_span: decl.span,
                });
            }
            ...
            Item::Let(decl) => {
                ...
                table.diagnostics.extend(d);
                table.define_global(decl.name, Symbol {
                    ty, kind: SymbolKind::Let, def_span: decl.span,
                });
            }
        }
    }
...
fn register_tool_decl(table: &mut SymbolTable, decl: &ToolDecl) {
    ...
    table.define_global(decl.name, Symbol {
        ty, kind: SymbolKind::Tool, def_span: decl.span,
    });
}
...
fn register_tool_entry(table: &mut SymbolTable, entry: &ToolEntry, d: &mut Vec<Diagnostic>) {
    ...
    table.define_global(entry.name, Symbol {
        ty, kind: SymbolKind::Tool, def_span: entry.span,
    });
}
...
fn validate_bound_params(
    ...
    for param in &decl.params {
        if param.binding != ParamBinding::BoundRef { continue; }
...
mod tests {
    ...
    fn interner() -> Arc<Interner> { Arc::new(Interner::new()) }
    fn sp() -> Span { Span::new(0, 10) }

    fn intern(i: &Arc<Interner>, s: &str) -> InternedStr { i.intern(s) }
    ...
    fn bound_param(i: &Arc<Interner>, name: &str) -> Param {
        Param { binding: ParamBinding::BoundRef, name: intern(i, name),
                ty: ty_str(i), optional: false, span: sp() }
    }

    fn empty_module() -> Module { Module { items: vec![], span: sp() } }
    ...
    fn type_decl_registers_record() {
        ...
        let module = Module {
            items: vec![Item::Type(TypeDecl {
                ...
                fields: vec![
                    FieldDef { name: intern(&i, "msg"), ty: ty_str(&i),
                               optional: false, span: sp() },
                ],
                span: sp(),
            })],
            span: sp(),
        ...
        let sym = t.globals.get(&intern(&i, "Response")).expect("Response not found");
    ...
    fn model_decl_registers_provider() {
        ...
        let module = Module {
            items: vec![Item::Model(ModelDecl {
                ...
                provider: ProviderCall {
                    ...
                    model_name: StringLit { value: intern(&i, "gemini-pro"),
        ...
        let t = build_symbol_table(&module);
        let sym = t.globals.get(&intern(&i, "MyModel")).expect("MyModel not found");
    ...
    fn agent_decl_registers_agent_ty() {
        ...
        let t = build_symbol_table(&module);
        let sym = t.globals.get(&intern(&i, "ChatAgent")).expect("ChatAgent not found");
    ...
    fn function_decl_registers_function_ty() {
        ...
        let t = build_symbol_table(&module);
        let sym = t.globals.get(&intern(&i, "greet")).expect("greet not found");
    ...
    fn duplicate_name_produces_diagnostic() {
        ...
        let decl = || Item::Type(TypeDecl {
            name: intern(&i, "Foo"), fields: vec![], span: sp(),
        });
        let module = Module { items: vec![decl(), decl()], span: sp() };
        let t = build_symbol_table(&module);
    ...
    fn function_with_tool_annotation_registers_tool_ty() {
        ...
        let tool_ann = Annotation {
            ...
            args: AnnotationArgs::Params(vec![
                Param { binding: ParamBinding::Normal, name: id_name,
                        ty: ty_str(&i), optional: false, span: sp() },
            ]),
            span: sp(),
        };
        ...
        let t = build_symbol_table(&module);
        assert!(t.diagnostics.is_empty(), "unexpected diags: {:?}", t.diagnostics);
        let sym = t.globals.get(&intern(&i, "deleteUser")).expect("deleteUser not found");
        if let Ty::Tool(tt) = &sym.ty {
            assert_eq!(tt.bound_params.len(), 1, "should have 1 bound param");
            assert_eq!(tt.model_params.len(), 1, "should have 1 model param");
        } else { panic!("expected Ty::Tool, got {:?}", sym.ty); }
    }
    ...
    fn bound_param_missing_from_annotation_produces_error() {
        ...
        let t = build_symbol_table(&module);
        assert!(!t.diagnostics.is_empty(), "expected error for unmatched bound param");
        assert_eq!(t.diagnostics[0].severity, Severity::Error);
    }
