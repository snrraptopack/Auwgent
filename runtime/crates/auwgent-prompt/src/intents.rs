use auwgent_ir_schema::{
    AgentIR, ComponentChildrenIR, ComponentIR as ComponentDefinition,
    ToolIR, TypeDeclIR as TypeDefinition, WorkflowIR,
};
use auwgent_schema::{
    flatten_example_object, flatten_helper_input_specs, flatten_named_field_specs,
    flatten_output_specs, FlatFieldSpec,
};
...
fn collect_helper_example_blocks(
    helpers: &[auwgent_ir_schema::HelperIR],
    types: Option<&HashMap<String, TypeDefinition>>,
) -> Vec<String> {
    let mut blocks = Vec::new();
...
fn collect_output_schema_entries(
    output: &Value,
    types: Option<&HashMap<String, TypeDefinition>>,
) -> Vec<(String, Vec<FlatFieldSpec>)> {
    let mut entries: Vec<(String, Vec<FlatFieldSpec>)> =
...
fn format_flat_field_specs(specs: &[FlatFieldSpec]) -> String {
    specs
        .iter()
        .map(|spec| {
...
fn format_component_signature(
    ...
    if let Some(children) = &component.children {
        let child_desc = match children {
            ComponentChildrenIR::All => "children: all".to_string(),
            ComponentChildrenIR::Only { components } => {
