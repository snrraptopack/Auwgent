use auwgent_ir_schema::{
    AgentIR, ConditionIR as Condition, ExpressionIR as Expression,
    ModelConfigIR as ModelConfig, ModelProviderIR as ModelProvider,
};
use auwgent_runtime_core::{AuwgentError, AuwgentResult};
use auwgent_schema::{format_output_schema_blocks, format_schema};
...
impl<'a> Evaluator<'a> {
    ...
    fn collect_context_references_from_expr(
        ...
        match expr {
            ...
            | Expression::If {
                ...
                if let Ok(cond) =
                    serde_json::from_value::<Condition>(condition.0.clone())
                {
            ...
            Expression::SchemaDirective { path } => match path.as_str() {
                ...
                "output" => {
                    if let Some(schema) = &self.ir.output {
                        Ok(Value::String(format_output_schema_blocks(
                            &schema.0,
                            self.ir.types.as_ref(),
                        )))
                    } else {
    ...
    pub fn evaluate_provider(
        ...
        match provider {
            ModelProvider::Gemini { model_name, config } => {
            ...
            ModelProvider::OpenAI { model_name, config } => {
                let mut res = serde_json::Map::new();
            ...
            ModelProvider::Groq { model_name, config } => {
                let mut res = serde_json::Map::new();
    ...
    fn evaluate_condition(
        ...
        match cond {
            Condition::Comparison(cmp) => {
                ...
                let op = cmp.operator.as_str();
                match op {
                    "==" => Ok(left == right),
                    "!=" => Ok(left != right),
                    ">" | ">=" | "<" | "<=" => {
                        // Try numeric comparison first
                        let left_num = left.as_f64().or_else(|| {
                            left.as_str().and_then(|s| s.parse::<f64>().ok())
                        });
                        let right_num = right.as_f64().or_else(|| {
                        ...
                        if let (Some(l), Some(r)) = (left_num, right_num) {
                            ...
                            let l_str = left.as_str().map(|s| s.to_string()).unwrap_or_else(|| left.to_string());
                            let r_str = right.as_str().map(|s| s.to_string()).unwrap_or_else(|| right.to_string());
                            match op {
                                ">" => Ok(l_str > r_str),
                                ">=" => Ok(l_str >= r_str),
                                "<" => Ok(l_str < r_str),
            ...
            Condition::ContextRef { property } => {
                if let Some(Value::Object(ctx)) = scope.get("context") {
                    Ok(ctx.contains_key(property))
                } else {
                    Ok(false)
                }
            }
            Condition::Logical { left, right, op } => {
                let left_result = self.evaluate_condition(left, scope)?;
                match op.as_str() {
                    "&&" => {
                        if !left_result {
                            return Ok(false);
    ...
    fn value_to_prompt_string(&self, val: &Value) -> String {
        match val {
