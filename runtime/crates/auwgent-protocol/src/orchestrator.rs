use auwgent_ir_schema::{ComponentIR as ComponentDefinition, TypeDeclIR as TypeDefinition};
use auwgent_schema::{
...
fn build_render_component_intent(
    ...
    if !component_instances.is_empty() {
        let registry: Map<String, Value> = component_instances
            ...
            .collect();
        intent.insert("registry".to_string(), Value::Object(registry));
    }
...
fn resolve_component_instance(
    ...
    let instance = instances.get(instance_id)?.clone();
    let mut resolved = instance.clone();

    if let Value::Object(ref mut map) = resolved {
        if let Some(Value::Array(children)) = map.get("children").cloned() {
            let mut resolved_children = Vec::new();
            for child in children {
                if let Some(child_id) = child.as_str() {
                    if let Some(child_tree) =
                        resolve_component_instance(child_id, instances, visited)
                    {
                        resolved_children.push(child_tree);
                    }
                } else {
                    resolved_children.push(child);
                }
            }
            map.insert("children".to_string(), Value::Array(resolved_children));
        }
    }
...
fn ast_to_json_object(ast_map: &std::collections::HashMap<String, ASTValue>) -> Value {
    let mut map = Map::new();
    for (k, v) in ast_map {
