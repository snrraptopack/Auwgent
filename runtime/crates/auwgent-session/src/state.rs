use auwgent_runtime_core::{Message, MessageContent, Role};
...
impl SessionState {
    ...
    pub fn set_turn_protocol(&mut self, protocol: impl Into<String>) {
        if let Some(turn) = self.turns.last_mut() {
    ...
    pub fn set_native_assistant_turn(&mut self, nat: NativeAssistantTurn) {
        if let Some(turn) = self.turns.last_mut() {
    ...
    pub fn append_native_tool_result(&mut self, result: NativeToolResult) {
        if let Some(turn) = self.turns.last_mut() {
