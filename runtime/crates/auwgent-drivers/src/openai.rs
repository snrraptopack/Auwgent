use crate::{ModelDriver, ModelEventStream};
use auwgent_runtime_core::{ModelEvent, TokenUsage, FinishReason};
use auwgent_runtime_core::{Message, Role};
                    ...
                    while let Some(index) = buffer.iter().position(|byte| *byte == b'\n') {
                            ...
                            && let Ok(json_val) = serde_json::from_str::<Value>(data)
                        {
                            ...
                            if let Some(usage) = json_val.get("usage") {
                                ...
                                result_events.push(ModelEvent::Usage(
                                    TokenUsage {
                                        prompt_tokens,
                                        completion_tokens,
                                        total_tokens,
