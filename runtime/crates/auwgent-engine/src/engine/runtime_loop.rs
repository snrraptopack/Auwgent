...
use super::*;
use auwgent_session::{display_input_value, input_parts_value};
...
impl AuwgentEngine {
    ...
    pub(super) async fn fire_middleware_event(&self, event: Value) -> Option<Value> {
        ...
        auwgent_middleware::fire_middleware_event(handler, event).await
    }
    ...
    pub(super) async fn apply_intent_middleware(
        ...
        auwgent_middleware::apply_intent_middleware(handler, payload).await
    }
    ...
    pub async fn begin_manual_run(
        ...
        if let Some(response) = auwgent_middleware::apply_run_start_middleware(handler, payload).await
    ...
    pub async fn apply_manual_llm_start(&self, prompt: String) -> AuwgentResult<String> {
        ...
        if let Some(middleware_result) =
            auwgent_middleware::apply_llm_start_middleware(handler, payload).await
        {
            let parsed = auwgent_middleware::parse_llm_start_response(&middleware_result);
    ...
    pub async fn apply_manual_llm_end(&self, response: Value) -> AuwgentResult<()> {
        ...
        let handler = self.middleware_event_handler.lock().unwrap().clone();
        auwgent_middleware::notify_llm_end_middleware(
            handler,
            &response,
    ...
    pub async fn complete_manual_run(&self) -> AuwgentResult<Value> {
        ...
        let _ = auwgent_middleware::apply_run_complete_middleware(handler, payload).await;
    ...
    pub async fn apply_manual_error(
        ...
        let response = auwgent_middleware::apply_error_middleware(handler, payload).await;
        Ok(response
            .as_ref()
        ...
        let run_result: AuwgentResult<()> = async {
            ...
            loop {
                ...
                *self.final_response_emitted.lock().unwrap() = false;
                ...
                let mut stream = match stream_res {
                    ...
                    Err(error) => {
                        ...
                        let middleware_response =
                            auwgent_middleware::apply_error_middleware(handler, payload).await;
                        let decision = middleware_response
                            .as_ref()
                            .map(|r| auwgent_middleware::parse_error_response(r))
                            .unwrap_or_default();
                ...
                let mut native_tool_calls: Vec<auwgent_session::NativeToolCallRecord> =
                    Vec::new();
                    ...
                    match chunk_res {
                        ...
                        Ok(ModelEvent::NativeToolCall {
                            ...
                            if let Some((action_kind, canonical_name)) =
                                auwgent_native::NativeCallableRegistry::route(
                                    &provider_name,
                                )
                            {
                                let canonical_name = canonical_name.to_string();
                                let intent_name = match action_kind {
                                    auwgent_native::ActionKind::Tool => {
                                        "tool_call"
                                    }
                                    auwgent_native::ActionKind::Workflow => {
                                        "workflow_call"
                                    }
                                    auwgent_native::ActionKind::Helper => {
                                        "helper_call"
                                    }
                                };
                                ...
                                native_tool_calls.push(
                                    auwgent_session::NativeToolCallRecord {
                                        id,
                                        provider_name,
                                        canonical_name,
                ...
                if is_native {
                    ...
                    if !native_tool_calls.is_empty() || native_structured_output.is_some() {
                        session.set_native_assistant_turn(
                            auwgent_session::NativeAssistantTurn {
                ...
                if is_native {
                    ...
                    for (result_key, args, result) in results {
                        ...
                        session.append_native_tool_result(
                            auwgent_session::NativeToolResult {
                                call_id,
                                provider_name,
                                canonical_name,
