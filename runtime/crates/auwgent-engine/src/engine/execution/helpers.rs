...
impl AuwgentEngine {
    pub(super) fn execute_helper<'a>(
        &'a self,
        call: &'a Value,
    ) -> auwgent_runtime_core::EngineFuture<'a, AuwgentResult<(String, Value, Value)>> {
        let session_preload_handler = self.session_preload_handler.clone();
        let session_save_handler = self.session_save_handler.clone();

        Box::pin(async move {
            use crate::helper_runner::build_sub_agent_context;
use auwgent_ir_schema::HandoffKindIR as HandoffMode;
