...
#[unsafe(no_mangle)]
pub extern "C" fn auwgent_engine_on_llm_start(
    handle: *mut EngineHandle,
    callback: Option<AuwgentLlmStartCallback>,
    free_result: Option<AuwgentFreeCallback>,
    user_data: *mut c_void,
) -> bool {
    clear_last_error();
    match with_bridge(handle, |bridge| {
        let callback = callback.ok_or_else(|| "llm start callback was null".to_string())?;
        let registration = JsonCallbackRegistration {
            callback,
            free_result,
            user_data,
        ...
        let handler: ir_runtime::runtime::engine::AsyncLlmStartCallback =
            Arc::new(move |input_json: String, system_prompt: String, context_json: String| {
                let registration = registration;
                Box::pin(async move {
                    let response = registration
                        .invoke_llm_start(&input_json, &system_prompt, &context_json)
                        .ok()
                        .flatten();
                    match response {
        ...
        bridge.engine.on_llm_start(handler);
        Ok(())
    }) {
        Ok(()) => true,
        Err(err) => {
...
#[unsafe(no_mangle)]
pub extern "C" fn auwgent_engine_on_llm_end(
    handle: *mut EngineHandle,
    callback: Option<AuwgentLlmEndCallback>,
    user_data: *mut c_void,
) -> bool {
    clear_last_error();
    match with_bridge(handle, |bridge| {
        let callback = callback.ok_or_else(|| "llm end callback was null".to_string())?;
        let registration = JsonCallbackRegistration {
            callback,
            free_result: None,
            user_data,
        ...
        let handler: ir_runtime::runtime::engine::AsyncLlmEndCallback =
            Arc::new(move |raw_response: String, system_prompt: String| {
                let registration = registration;
                Box::pin(async move {
                    let _ = registration.invoke_llm_end(&raw_response, &system_prompt);
                })
            });

        bridge.engine.on_llm_end(handler);
        Ok(())
    }) {
        Ok(()) => true,
        Err(err) => {
...
#[unsafe(no_mangle)]
pub extern "C" fn auwgent_engine_on_run_start(
    handle: *mut EngineHandle,
    callback: Option<AuwgentSessionTransformCallback>,
    free_result: Option<AuwgentFreeCallback>,
    user_data: *mut c_void,
    ...
    clear_last_error();
    match with_bridge(handle, |bridge| {
        let callback = callback.ok_or_else(|| "run start callback was null".to_string())?;
        let registration = JsonCallbackRegistration {
            callback,
            free_result,
            user_data,
        ...
        let handler: ir_runtime::runtime::engine::AsyncRunStartCallback =
            Arc::new(move |session_json: String, context_json: String| {
                let registration = registration;
                Box::pin(async move {
                    registration
                        .invoke_session_transform(&session_json, &context_json)
                        .ok()
                        .flatten()
                })
            });

        bridge.engine.on_run_start(handler);
        Ok(())
    }) {
        Ok(()) => true,
        Err(err) => {
...
#[unsafe(no_mangle)]
pub extern "C" fn auwgent_engine_on_run_complete(
    handle: *mut EngineHandle,
    callback: Option<AuwgentSessionNotifyCallback>,
    user_data: *mut c_void,
) -> bool {
    clear_last_error();
    match with_bridge(handle, |bridge| {
        let callback = callback.ok_or_else(|| "run complete callback was null".to_string())?;
        let registration = JsonCallbackRegistration {
            callback,
            free_result: None,
            user_data,
        ...
        let handler: ir_runtime::runtime::engine::AsyncRunCompleteCallback =
            Arc::new(move |session_json: String, context_json: String| {
                let registration = registration;
                Box::pin(async move {
                    let _ = registration.invoke_session_notify(&session_json, &context_json);
                })
            });

        bridge.engine.on_run_complete(handler);
        Ok(())
    }) {
        Ok(()) => true,
        Err(err) => {
...
#[unsafe(no_mangle)]
pub extern "C" fn auwgent_engine_on_error(
    handle: *mut EngineHandle,
    callback: Option<AuwgentErrorCallback>,
    user_data: *mut c_void,
) -> bool {
    clear_last_error();
    match with_bridge(handle, |bridge| {
        let callback = callback.ok_or_else(|| "error callback was null".to_string())?;
        let registration = JsonCallbackRegistration {
            callback,
            free_result: None,
            user_data,
        ...
        let handler: ir_runtime::runtime::engine::AsyncErrorCallback =
            Arc::new(move |error_json: String, session_json: Option<String>, context_json: String| {
                let registration = registration;
                Box::pin(async move {
                    registration
                        .invoke_error(&error_json, session_json.as_deref(), &context_json)
                        .unwrap_or(false)
                })
            });

        bridge.engine.on_error(handler);
        Ok(())
    }) {
        Ok(()) => true,
        Err(err) => {
