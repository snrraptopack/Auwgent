...
grammars, and static-metadata-driven schema mapping. We describe six classes
of malformed intent-stream patterns, directly quantify five, and report a
cumulative ablation isolating the scanner, tolerant lexer, and compiler-bound
...
\item An execution-safety policy coupling compiler-authorized structural
reconstruction to ambiguity rejection and partial-state execution gating.
\item A characterization of six malformed-stream patterns and their recovery
mechanisms, five of which are directly quantified in the present corpus.
...
\node[left=6mm of l5, font=\tiny, rotate=90, anchor=center] {Validated Intent AST out};
\end{tikzpicture}%
}
...
\paragraph{Operational summary.} For each chunk, Layer 2 appends text to its
buffer, extracts only complete recognized headers, and retains any suffix that
is a known header prefix; finalization alone enables newline-bounded header
repair. For each parsed key--value pair, Layer 4 first tests the exact alias
map, then tests a dotted path against compiler-authorized paths, otherwise
preserving the key literally. Before reconstruction, any duplicate in a
completed payload changes the decision from acceptance to rejection.
...
We describe six malformed-stream patterns handled by the architecture. Five
are directly exercised by the fault-directed evaluation; Pattern 2 is an
implemented behavior documented here but not independently quantified.
...
deserialization exception. No standalone corpus stratum is assigned to this
...
\end{aligned}
\]
where $\mathcal{K}_{\mathrm{IMP}}$ is the set of registered IMP intent kinds,
$N$ is a target name or metadata, $P$ is payload text, and $C(k)$ is the
registered closing marker for kind $k$. Recovery maps malformed
...
quantified coverage is claimed only for the five directly evaluated patterns,
not for Pattern 2 or arbitrary corruption.
...
\node[below=3mm of buf, font=\tiny, align=center]
  {Resolved on final fragment: \texttt{tool\_call = create\_user}\\emitted once, not per chunk};
\end{tikzpicture}%
}
...
v0.62.0 comparison is also run on Suites A, C, and H. Because it does not
...
With $N_e$ exact positive-oracle matches, $N_r$ recoverable cases, $N_a$
ambiguity controls accepted as a specific AST, and $N_c$ total controls:
\[
\mathrm{SRR}=100\frac{N_e}{N_r}, \qquad
\mathrm{OTR}=100\frac{N_a}{N_c}.
\]
...
stream.
\item \textbf{Over-Tolerance Rate (OTR, new):} \% of all 150 ambiguity controls silently
...
the full IMP parser. On positive Suite H payloads it recovered 50/150
(33.33\%); all 50 were already-valid JSON bodies paired with malformed headers
that the payload-only baseline did not receive. It failed the email and dotted
categories and accepted all 50 Suite H duplicate controls (100.00\% OTR), so
this result is not evidence of envelope recovery. On all 150 conflicting
duplicate-key controls, strict
