...
establishing compatibility for those tested models and token-scaling behavior while leaving
the downstream parser mechanisms unisolated \cite{irai2026}. This paper opens
that box. We present a five-layer
...
follow-up.'' Before that comparison can be interpreted, the post-hoc parser's
...
\item A cumulative layer-wise ablation isolating the scanner, tolerant lexer,
and schema-unflattening contributions on a 900-case synthetic suite, including
150 ambiguity controls, with a separate 4,000-trial sweep of streaming-layer
transport fragmentation.
\item An analysis of over-tolerance: cases where a parsing configuration accepts
ambiguous input as valid rather than failing, and the correctness risk
this introduces -- a failure mode we argue is under-reported in prior
tolerant-parsing work.
\end{enumerate}
...
\textbf{Application-layer post-hoc parsing}, the subject of this paper,
...
\midrule
Constrained decoding & Generation-time decoder & Decoder-specific & Prevent grammar violations \\
Provider-native & Vendor service & Provider-specific & Provider-managed \\
Post-hoc IMP & Client runtime & Yes & Bounded recovery \\
\bottomrule
\end{tabular}}
\end{table}
...
The problem of recovering a usable parse tree from syntactically invalid
input predates LLMs by decades. Classical compiler error-recovery
techniques -- panic-mode recovery, phrase-level repair, and LR/LL error
diagnosis \cite{burkefisher} -- establish that a parser can continue
past a syntax violation by locally resynchronizing on a recognizable
token rather than aborting. \emph{Incremental parsing} literature
\cite{wagnergraham}, and its modern realization in editor tooling such
as tree-sitter \cite{treesitter}, further establishes that a parser can
maintain a valid partial tree while input is still arriving, re-parsing
only the region touched by new input. Layer 2 and Layer 3 of the IMP
parser (Sections~III-B, III-C) apply this lineage to a new setting: the
``invalid'' input is not a typo in source code but a structurally
plausible LLM completion, and the resynchronization points are IMP
delimiter tokens rather than statement boundaries. Unlike tree-sitter,
which optimizes for minimal re-parse cost under keystroke-level edits,
the IMP scanner optimizes for zero-leakage buffering under
network-level chunk boundaries -- a distinct failure mode (Pattern 5,
Section~IV) that arises specifically from HTTP streaming transport
rather than interactive editing.

\subsection{Indentation-Sensitive and Tolerant Lexical Grammars}

Layer 3's layout-sensitive fallback is not a novel lexical strategy in
isolation: YAML \cite{yamlspec} and Python demonstrate that
whitespace-delimited structure can substitute for explicit braces, and
JSON5 \cite{json5spec} demonstrates that relaxing JSON's quoting and
trailing-comma rules does not require abandoning JSON's data model. The
contribution here is not the existence of a tolerant grammar but its
composition into a fallback \emph{cascade} selected dynamically per
payload (brace-delimited TypeScript-object parsing, then layout parsing,
then semantic token protection for embedded \texttt{@}/\texttt{.}
characters), driven by which stage first fails rather than a single
fixed grammar chosen ahead of time.

\subsection{Schema-Aware Post-Hoc Parsing}
...
Layer 4's unflattening is architecturally closer to schema-mapping
systems than to parsing per se. Protocol Buffers \cite{protobuf} and
GraphQL \cite{graphqlspec} both compile a schema ahead of time and use
that compiled metadata to interpret wire-level data at runtime; ORM
field-mapping layers perform an analogous flat-to-nested translation
between relational rows and object graphs. The IMP parser applies the
same principle in the opposite direction expected by those systems: prior
work compiles schema metadata to \emph{validate or serialize} already
well-formed data, whereas Layer 4 uses compiler-produced exact aliases and
authorized paths to reconstruct flat output. Relative to SAP's broader
schema-aligned correction, the contribution evaluated here is the composition
of exact compiler mapping with an IMP envelope scanner, transport-aware
finalization, duplicate-key rejection, and a rule that partial recovery cannot
authorize execution.
...
all post-hoc structured-output parsers.
...
\paragraph{Bounded recovery contract.} Final-stream header repair requires a
newline boundary; dotted expansion requires an exact compiler-known path;
duplicate keys reject the completed block; and prefix recovery may update
partial UI state but cannot authorize tool execution.
...
present ablation deliberately holds generation outside the experiment so
parser-layer effects are measured independently. The artifact can import
provenance-labeled JSONL generations, but no new model-scale result is claimed.
Thus, the preceding study supplies end-to-end external evidence for the tested
models and tasks, while this study supplies mechanism-level internal evidence.
The combination does not estimate the field frequency of individual corruption
patterns, and the reported SRR should be read as conditional recovery coverage.
...
\item \textbf{First-Pass Execution Rate:} not reported because the
micro-benchmark invokes no tool executor or corrective LLM loop.
\item \textbf{Parser TTFPS:} local elapsed time from supplying the first
chunk to the first valid partial-intent callback. This excludes generation,
network, rendering, and tool-execution latency.
\item \textbf{Prompt Token Footprint Reduction:} vs. full OpenAPI/JSON
Schema injection (values already established in \cite{irai2026}; reused
here for continuity, not re-measured).
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Configuration & Syntax A & Protocol B & Structure C & Holdout H \\
\midrule
...
holdout case. End-of-stream header repair is conservative: it uses a line
...
on Suites A and C. On the 150 conflicting duplicate-key controls, strict
...
The evaluation isolates deterministic parser behavior, not end-to-end agent
reliability. Its synthetic generators cover named corruption families but are
not a probability sample of real LLM failures; consequently, SRR is not a
forecast of the parser's deployment-wide success rate. Real-model compatibility and
token scaling across Qwen-3-32B, Llama-3.3-70B, and GPT-OSS-120B were evaluated
in \cite{irai2026}; they are not re-run here because the present experiment
holds generation constant to isolate parser layers. Consequently, this paper
adds no new model-family generalization, BFCL/ToolBench accuracy, or true
first-pass execution claim. Prompt-token reduction is likewise inherited from
\cite{irai2026}, and no generation-layer constrained-decoder or provider-native
latency baseline was executed.
...
all 150 duplicate-key controls. These results support post-hoc parsing as a
