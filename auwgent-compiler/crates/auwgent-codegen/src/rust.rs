...
pub fn generate(plan: &CodegenPlan, base_name: &str) -> String {
    ...
    sections.push(generate_output_type(
        agent_name,
        ir.get("output"),
        &schema_output_helpers,
    ));
    if has_context {
        sections.push(generate_named_shape(CONTEXT_TYPE, ir.get("context")));
    }
    if has_tools {
        sections.push(generate_tools(agent_name, all_tools, &mut emitted_shapes));
    }
    ...
    let declared_type_names = ir
        .get("types")
        .and_then(Value::as_object)
        .map(|types| types.keys().cloned().collect::<Vec<_>>())
        .unwrap_or_default();
    sections.push(generate_core_intents(
        agent_name,
        ir.get("output"),
        &schema_output_helpers,
        &declared_type_names,
    ...
    if has_tools {
        sections.push(generate_callable_family(
            agent_name,
            "Tool",
            all_tools,
            "name",
    ...
    if has_components {
        sections.push(generate_component_intents(agent_name));
    }
    sections.push(generate_top_level_intent_enums(
        agent_name,
        has_tools,
        has_workflows,
        has_helpers,
    ...
    sections.push(generate_decode_functions(
        agent_name,
        has_tools,
        has_workflows,
        has_helpers,
...
fn generate_helper_output_type(helper: &Value) -> String {
    ...
    match helper.get("output") {
        None | Some(Value::Null) => format!("pub type {helper_name}Output = ();\n"),
        Some(output) => generate_named_shape(
            &format!("{helper_name}Output"),
            unwrap_wrapped_shape(Some(output)).or(Some(output)),
        ),
    }
}
...
fn generate_output_type(
    _agent_name: &str,
    value: Option<&Value>,
    output_helpers: &[Value],
) -> String {
    let Some(value) = value else {
        if output_helpers.is_empty() {
            return format!("pub type {OUTPUT_TYPE} = JsonValue;\n");
        }
    ...
    if let Some(variants) = value.get("__variants").and_then(Value::as_object) {
        ...
        for (variant_name, variant_props) in variants {
            ...
            enum_variants.push(format!(
                ...
                to_rust_type_name(variant_name)
            ));
        }
    ...
    if output_helpers.is_empty() {
        return generate_named_shape(OUTPUT_TYPE, Some(value));
    }
    let mut blocks = Vec::new();
    let mut enum_variants = Vec::new();
    if !value.is_null() {
        ...
        enum_variants.push(format!("    Base({base_name}),"));
    }
    for helper in output_helpers {
        if let Some(helper_name) = string_at(helper, &["name"]) {
            let helper_output = format!("{}Output", helper_name);
...
fn generate_struct(name: &str, properties: &Map<String, Value>) -> String {
    ...
    for (prop_name, prop_info) in properties {
        ...
        let rename = if field_name != prop_name.as_str() {
            format!("    #[serde(rename = \"{prop_name}\")]\n")
        } else {
            String::new()
        };
...
fn generate_tools(
    agent_name: &str,
    tools: &[Value],
    emitted_shapes: &mut BTreeSet<String>,
) -> String {
    let _ = agent_name;
    ...
    for tool in tools {
        ...
        let args_type = shape_type_or_no_args(
            &format!("{pascal}Args"),
            tool.get("params"),
            &mut result_aliases,
            emitted_shapes,
        ...
        if args_type == "NoArgs" {
            ...
            methods.push(format!(
                "    fn {method_name}(&self, args: {pascal}Args) -> {pascal}Result;"
            ));
        }
...
fn generate_custom_intent_types(agent_name: &str, plan: &CodegenPlan) -> String {
    let _ = agent_name;
    let mut blocks = Vec::new();
    for (name, item) in plan.custom_intent_defs() {
        blocks.push(generate_named_shape(
            &format!("{}Intent", to_rust_type_name(name)),
            item.get("fields"),
        ));
    }
    blocks.join("\n")
...
fn generate_core_intents(
    ...
    let mut response_value_arms = Vec::new();
    let root_schema_name = format!("{agent_name}Output");
    let has_root_output = output.is_some_and(|value| !value.is_null());
    if has_root_output {
        response_variants.push(format!("    #[serde(rename = \"{root_schema_name}\")]\n    AuwgentOutput {{\n        response: {OUTPUT_TYPE},\n    }},"));
        response_value_arms.push(format!("            {RESPONSE_SCHEMA_TYPE}::AuwgentOutput {{ response }} => serde_json::to_value(response.clone()),"));
    }
    for helper in output_helpers {
        if let Some(helper_name) = string_at(helper, &["name"]) {
            let variant_name = to_rust_type_name(helper_name);
            let schema_name = format!("{helper_name}Output");
            let helper_output_type = format!("{helper_name}Output");
            response_variants.push(format!("    #[serde(rename = \"{schema_name}\")]\n    {variant_name}Output {{\n        response: {helper_output_type},\n    }},"));
            response_value_arms.push(format!("            {RESPONSE_SCHEMA_TYPE}::{variant_name}Output {{ response }} => serde_json::to_value(response.clone()),"));
        }
    }
    for declared_type_name in declared_type_names {
        let variant_name = to_rust_type_name(declared_type_name);
        let rust_type_name = to_rust_type_name(declared_type_name);
        response_variants.push(format!("    #[serde(rename = \"{declared_type_name}\")]\n    {variant_name} {{\n        response: {rust_type_name},\n    }},"));
        response_value_arms.push(format!("            {RESPONSE_SCHEMA_TYPE}::{variant_name} {{ response }} => serde_json::to_value(response.clone()),"));
    }
    let root_type_arm = if has_root_output {
        format!(
            "            {RESPONSE_SCHEMA_TYPE}::AuwgentOutput {{ .. }} => \"{root_schema_name}\",\n"
        )
    } else {
        String::new()
    };
    let response_schema = format!(
        "#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]\n#[serde(tag = \"type\")]\npub enum {RESPONSE_SCHEMA_TYPE} {{\n{}\n}}\n\nimpl {RESPONSE_SCHEMA_TYPE} {{\n    pub fn response_value(&self) -> JsonValue {{\n        match self {{\n{}\n        }}.expect(\"response should serialize\")\n    }}\n\n    pub fn type_name(&self) -> &'static str {{\n        match self {{\n{}{}\n        }}\n    }}\n}}\n",
        response_variants.join("\n"),
        response_value_arms.join("\n"),
        root_type_arm,
        output_helpers
            .iter()
            .filter_map(
                |helper| string_at(helper, &["name"]).map(|helper_name| format!(
                    "            {RESPONSE_SCHEMA_TYPE}::{}Output {{ .. }} => \"{}Output\",",
                    to_rust_type_name(helper_name),
                    helper_name
                ))
            )
            .chain(declared_type_names.iter().map(|name| format!(
...
fn generate_callable_family(
    ...
    for item in items {
        ...
        let args_type = shape_type_or_no_args(
            &format!("{pascal}{family_name}Args"),
            item.get(args_key),
            &mut blocks,
            emitted_shapes,
        );
        let result_type = if family_name == "Helper" {
            format!("{pascal}Output")
        } else {
            rust_type(item.get(result_key), false, "()")
        };
        ...
        if include_error_and_skipped {
            skipped_variants.push(enum_struct_variant(item_name, &pascal, "args", &args_type));
        }
    }
    blocks.push(format!("#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]\n#[serde(tag = \"type\")]\npub enum {call_name} {{\n{}\n}}\n",call_variants.join("\n")));
...
fn enum_struct_variant(
    ...
    if field_type == "NoArgs" {
        format!("    #[serde(rename = \"{item_name}\")]\n    {pascal},")
    } else {
        format!(
...
fn generate_component_intents(agent_name: &str) -> String {
    ...
    [
        ...
        "pub type RenderComponentIntent = JsonValue;\n".to_string(),
    ]
    .join("\n")
}

fn generate_top_level_intent_enums(
    agent_name: &str,
    has_tools: bool,
    has_workflows: bool,
    has_helpers: bool,
    ...
    let _ = agent_name;
    let mut intent_variants = vec![
        format!("    ResponseText({RESPONSE_TEXT_TYPE}),"),
        format!("    ResponseSchema({RESPONSE_SCHEMA_TYPE}),"),
        format!("    Error({ERROR_INTENT_TYPE}),"),
    ];
    let mut partial_variants = vec![
        "    ResponseText(PartialTextIntentValue),".to_string(),
        format!("    ResponseSchema(PartialStructuredIntentValue<{RESPONSE_SCHEMA_TYPE}>),"),
        format!("    Error(PartialStructuredIntentValue<{ERROR_INTENT_TYPE}>),"),
    ];
    if has_tools {
        intent_variants.extend([
            "    ToolCall(ToolCall),".to_string(),
            "    ToolResult(ToolResult),".to_string(),
            "    ToolError(ToolError),".to_string(),
            "    ToolSkipped(ToolSkipped),".to_string(),
        ]);
        partial_variants.extend([
            "    ToolCall(PartialStructuredIntentValue<ToolCall>),".to_string(),
            "    ToolResult(PartialStructuredIntentValue<ToolResult>),".to_string(),
            "    ToolError(PartialStructuredIntentValue<ToolError>),".to_string(),
            "    ToolSkipped(PartialStructuredIntentValue<ToolSkipped>),".to_string(),
        ]);
    }
    if has_workflows {
        intent_variants.extend([
            "    WorkflowCall(WorkflowCall),".to_string(),
            "    WorkflowResult(WorkflowResult),".to_string(),
        ]);
        partial_variants.extend([
            "    WorkflowCall(PartialStructuredIntentValue<WorkflowCall>),".to_string(),
            "    WorkflowResult(PartialStructuredIntentValue<WorkflowResult>),".to_string(),
        ]);
    }
    if has_helpers {
        intent_variants.extend([
            "    HelperCall(HelperCall),".to_string(),
            "    HelperResult(HelperResult),".to_string(),
        ]);
        partial_variants.extend([
            "    HelperCall(PartialStructuredIntentValue<HelperCall>),".to_string(),
            "    HelperResult(PartialStructuredIntentValue<HelperResult>),".to_string(),
        ]);
    }
    if has_components {
        intent_variants.extend([
            "    Component(ComponentIntent),".to_string(),
            "    RenderComponent(RenderComponentIntent),".to_string(),
        ]);
        partial_variants.extend([
            "    Component(PartialStructuredIntentValue<ComponentIntent>),".to_string(),
            "    RenderComponent(PartialStructuredIntentValue<RenderComponentIntent>),".to_string(),
        ]);
    }
    for custom_intent in custom_intents {
        let pascal = to_rust_type_name(custom_intent);
        let type_name = format!("{pascal}Intent");
        intent_variants.push(format!("    {pascal}({type_name}),"));
        partial_variants.push(format!(
            "    {pascal}(PartialStructuredIntentValue<{type_name}>),"
        ));
    }
    format!(
...
fn generate_decode_functions(
    ...
    let mut name_cases = vec![
        format!("        \"response_text\" => Some({INTENT_NAME_ENUM}::ResponseText),"),
        format!("        \"response_schema\" => Some({INTENT_NAME_ENUM}::ResponseSchema),"),
        format!("        \"error\" => Some({INTENT_NAME_ENUM}::Error),"),
    ];
    let mut intent_cases = vec![
        format!(
            "        {INTENT_NAME_ENUM}::ResponseText => serde_json::from_value(value).ok().map({INTENT_ENUM}::ResponseText),"
        ),
        format!(
            "        {INTENT_NAME_ENUM}::ResponseSchema => serde_json::from_value(value).ok().map({INTENT_ENUM}::ResponseSchema),"
        ),
        format!(
            "        {INTENT_NAME_ENUM}::Error => serde_json::from_value(value).ok().map({INTENT_ENUM}::Error),"
        ),
    ];
    let mut partial_cases = vec![
        format!(
            "        {INTENT_NAME_ENUM}::ResponseText => serde_json::from_value(value).ok().map({INTENT_PARTIAL_ENUM}::ResponseText),"
        ),
        format!(
            "        {INTENT_NAME_ENUM}::ResponseSchema => serde_json::from_value(value).ok().map({INTENT_PARTIAL_ENUM}::ResponseSchema),"
        ),
        format!(
            "        {INTENT_NAME_ENUM}::Error => serde_json::from_value(value).ok().map({INTENT_PARTIAL_ENUM}::Error),"
        ),
    ];
    if has_tools {
        name_cases.extend([
            format!("        \"tool_call\" => Some({INTENT_NAME_ENUM}::ToolCall),"),
            format!("        \"tool_result\" => Some({INTENT_NAME_ENUM}::ToolResult),"),
            format!("        \"tool_error\" => Some({INTENT_NAME_ENUM}::ToolError),"),
            format!("        \"tool_skipped\" => Some({INTENT_NAME_ENUM}::ToolSkipped),"),
        ]);
    ...
    if has_workflows {
        name_cases.extend([
            format!("        \"workflow_call\" => Some({INTENT_NAME_ENUM}::WorkflowCall),"),
            format!("        \"workflow_result\" => Some({INTENT_NAME_ENUM}::WorkflowResult),"),
        ]);
    ...
    if has_helpers {
        name_cases.extend([
            format!("        \"helper_call\" => Some({INTENT_NAME_ENUM}::HelperCall),"),
            format!("        \"helper_result\" => Some({INTENT_NAME_ENUM}::HelperResult),"),
        ]);
    ...
    if has_components {
        name_cases.extend([
            format!("        \"component\" => Some({INTENT_NAME_ENUM}::Component),"),
            format!("        \"render_component\" => Some({INTENT_NAME_ENUM}::RenderComponent),"),
        ]);
    ...
    for custom_intent in custom_intents {
        let pascal = to_rust_type_name(custom_intent);
        name_cases.push(format!(
            "        \"{custom_intent}\" => Some({INTENT_NAME_ENUM}::{pascal}),"
        ));
        intent_cases.push(format!("        {INTENT_NAME_ENUM}::{pascal} => serde_json::from_value(value).ok().map({INTENT_ENUM}::{pascal}),"));
        partial_cases.push(format!("        {INTENT_NAME_ENUM}::{pascal} => serde_json::from_value(value).ok().map({INTENT_PARTIAL_ENUM}::{pascal}),"));
    }
    format!(
        "impl {INTENT_NAME_ENUM} {{\n    pub fn parse(name: &str) -> Option<Self> {{\n        match name {{\n{}\n            _ => None,\n        }}\n    }}\n}}\n\nimpl {INTENT_ENUM} {{\n    pub fn decode(name: {INTENT_NAME_ENUM}, value: JsonValue) -> Option<Self> {{\n        match name {{\n{}\n        }}\n    }}\n}}\n\nimpl {INTENT_PARTIAL_ENUM} {{\n    pub fn decode(name: {INTENT_NAME_ENUM}, value: JsonValue) -> Option<Self> {{\n        match name {{\n{}\n        }}\n    }}\n}}\n",
        name_cases.join("\n"),
        intent_cases.join("\n"),
        partial_cases.join("\n")
    )
...
fn generate_handler_traits(
    ...
    let mut dispatch_arms = vec![
        format!(
            "            {INTENT_ENUM}::ResponseText(value) => self.response_text(value, agent_name),"
        ),
        format!(
            "            {INTENT_ENUM}::ResponseSchema(value) => self.response_schema(value, agent_name),"
        ),
...
fn generate_api_keys(
    ...
    if required_providers.contains("openai") {
        ...
        init_fields.push("            openai_api_key: value.openai_api_key,".to_string());
    }
    if required_providers.contains("gemini") {
        fields.push("    pub gemini_api_key: Option<String>,".to_string());
        init_fields.push("            gemini_api_key: value.gemini_api_key,".to_string());
    }
    if required_providers.contains("groq") {
        fields.push("    pub groq_api_key: Option<String>,".to_string());
        init_fields.push("            groq_api_key: value.groq_api_key,".to_string());
    }
...
fn generate_config(
    ...
    let mut fields = Vec::new();
    if has_tools {
        fields.push("    pub tools: TTools,".to_string());
    }
    fields.push("    pub middleware: Vec<TMiddleware>,".to_string());
    if has_context {
        fields.push(format!("    pub context: {CONTEXT_TYPE},"));
    }
    if has_api_keys {
        fields.push(format!("    pub api_keys: {API_KEYS_TYPE},"));
    }
    let generics = if has_tools {
        format!("<TTools = {TOOLS_REGISTRY}, TMiddleware = {MIDDLEWARE_REGISTRY}>")
    } else {
        format!("<TMiddleware = {MIDDLEWARE_REGISTRY}>")
    };
    format!(
...
fn generate_agent(
    ...
    let inner_tools_type = if has_tools {
        TOOLS_REGISTRY.to_string()
    } else {
        "()".to_string()
    };
    let tools_value = if has_tools {
        "config.tools.into()".to_string()
    } else {
        "()".to_string()
    };
    let config_type = if has_tools {
        format!("{CONFIG_TYPE}<TTools, TMiddleware>")
    } else {
        format!("{CONFIG_TYPE}<TMiddleware>")
    };
    let generics = if has_tools {
        "<TTools, TMiddleware>"
    } else {
...
fn shape_properties(value: Option<&Value>, unwrap_input_kind: bool) -> Option<Map<String, Value>> {
    ...
    if let Some(inner) = obj.get("type").and_then(Value::as_object) {
        if inner.get("type").and_then(Value::as_str) == Some("object") {
            return inner.get("properties").and_then(Value::as_object).cloned();
        }
    }
    if unwrap_input_kind && obj.get("kind").and_then(Value::as_str) == Some("properties") {
...
fn shape_type_or_no_args(
    name: &str,
    shape: Option<&Value>,
    blocks: &mut Vec<String>,
    emitted_shapes: &mut BTreeSet<String>,
) -> String {
    let shape = unwrap_wrapped_shape(shape).or(shape);
...
fn rust_type_base(value: Option<&Value>, fallback: &str) -> String {
    ...
    let value = unwrap_wrapped_shape(Some(value)).unwrap_or(value);
...
fn unwrap_wrapped_shape<'a>(value: Option<&'a Value>) -> Option<&'a Value> {
    let value = value?;
    let obj = value.as_object()?;
...
mod tests {
    ...
    use serde_json::json;
    #[test]
    fn emits_typed_intents_and_conditional_config() {
        let ir = json!({"name":"Hello","context":{"user_id":{"type":"string","optional":false}},"tools":[{"name":"get_details","params":{},"returns":{"type":"typeRef","name":"Person"}},{"name":"get_location","params":{"id":{"type":"string","optional":false}},"returns":{"type":"string"}}],"helpers":[{"name":"Joker","input":null,"output":null,"customIntents":[{"name":"helper_prompt","fields":{"message":{"type":"string","optional":false}}}]}],"output":{"__variants":{"Output":{"name":{"type":"string","optional":false}},"Fallback":{"message":{"type":"string","optional":false}}}},"types":{"Person":{"properties":{"name":{"type":"string","optional":false},"age":{"type":"number","optional":false}}}},"customIntents":[{"name":"ask_user","fields":{"question":{"type":"string","optional":false}}}],"modelConfig":[{"defaultConfig":{"model":{"type":"openai","modelName":"gpt-4.1"}}}]});
        let output = generate(&CodegenPlan::new(ir), "hello");
        assert!(output.contains("pub enum AuwgentIntent"));
        assert!(output.contains("pub enum AuwgentIntentPartial"));
        assert!(output.contains("impl AuwgentIntentName"));
        assert!(output.contains("pub fn parse(name: &str) -> Option<Self>"));
        assert!(output.contains("impl AuwgentIntent {"));
        assert!(
            output.contains(
                "pub fn decode(name: AuwgentIntentName, value: JsonValue) -> Option<Self>"
            )
        );
        assert!(output.contains("impl AuwgentIntentPartial {"));
        assert!(output.contains("pub struct Intents"));
        assert!(output.contains("pub trait AuwgentIntentHandler"));
        assert!(output.contains("pub fn on_intent<H>(&self, handler: H)"));
        assert!(output.contains("pub fn on_intent_raw<F>(&self, handler: F)"));
        assert!(
            output.contains("on_decoded_intent(AuwgentIntentName::parse, AuwgentIntent::decode")
        );
        assert!(output.contains(
            "on_decoded_intent_partial(AuwgentIntentName::parse, AuwgentIntentPartial::decode"
        ));
        assert!(output.contains("fn tool_names(&self) -> &'static [&'static str]"));
        assert!(output.contains("fn invoke_tool("));
        assert!(output.contains("pub trait AuwgentTools: Send + Sync + 'static"));
        assert!(output.contains("impl<T> From<T> for AuwgentToolsRegistry"));
        assert!(!output.contains("impl AuwgentToolsRegistry"));
        assert!(output.contains("pub struct AuwgentConfig<TTools = AuwgentToolsRegistry, TMiddleware = AuwgentMiddlewareRegistry>"));
        assert!(output.contains("pub tools: TTools,"));
        assert!(output.contains("pub middleware: Vec<TMiddleware>,"));
        assert!(output.contains("let registry: AuwgentMiddlewareRegistry = item.into();"));
        assert!(output.contains("tools: config.tools.into(),"));
        assert!(output.contains("pub trait AuwgentMiddleware: Send + Sync + 'static"));
        assert!(!output.contains("pub use "));
        assert!(output.contains("pub struct HelperPromptIntent"));
        assert!(output.contains("pub struct AskUserIntent"));
        assert!(output.contains("AuwgentIntent::AskUser(..) => \"ask_user\""));
        assert!(
            output
                .contains("AuwgentIntent::AskUser(intent) => serde_json::to_value(intent.clone())")
        );
        assert!(output.contains("fn ask_user(&self, _value: &AskUserIntent, _agent: &str) {}"));
        assert!(
            output.contains("AuwgentIntent::AskUser(value) => self.ask_user(value, agent_name)")
        );
        assert!(output.contains("AuwgentIntent::HelperPrompt(..) => \"helper_prompt\""));
        assert!(
            output
                .contains("fn helper_prompt(&self, _value: &HelperPromptIntent, _agent: &str) {}")
        );
        assert!(!output.contains("use std::sync::{Arc, Mutex};"));
    }
    #[test]
    fn omits_conditional_fields_when_not_needed() {
        let ir = json!({"name":"Mini","tools":[],"helpers":[],"components":[],"modelConfig":[]});
        let output = generate(&CodegenPlan::new(ir), "mini");
        assert!(
            output.contains("pub struct AuwgentConfig<TMiddleware = AuwgentMiddlewareRegistry>")
        );
        assert!(output.contains("pub middleware: Vec<TMiddleware>,"));
        assert!(!output.contains("pub tools: TTools,"));
        assert!(!output.contains("pub context: AuwgentContext,"));
        assert!(!output.contains("pub api_keys: AuwgentApiKeys,"));
        assert!(output.contains("pub trait AuwgentIntentHandler"));
        assert!(output.contains("pub fn on_intent_raw<F>(&self, handler: F)"));
