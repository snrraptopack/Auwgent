...
pub fn run_compile(path: Option<PathBuf>, output: Option<PathBuf>, config: Option<PathBuf>) {
    ...
    let out = output.or_else(|| {
        cfg.as_ref()
            .and_then(|c| c.output.as_deref().map(PathBuf::from))
    });
    let files = resolution::resolve_sources(
        path.as_deref(),
        cfg.as_ref().and_then(|c| c.source.as_deref()),
    );
...
pub fn run_generate(
    ...
    if watch {
        ...
        crate::watch::watch_and_generate(
            &files,
            &watch_roots,
            &resolution::resolve_targets(
                target.as_deref(),
                cfg.as_ref().map(|c| c.targets.as_slice()),
            ),
            out.as_deref(),
        );
    } else {
        let root_dir: &Path = path
            .as_deref()
            .or_else(|| config_source.map(Path::new))
            .unwrap_or(Path::new("."));
        let mut projects: Vec<(PathBuf, Option<Config>)> =
            vec![(root_dir.to_path_buf(), cfg.clone())];
        ...
        for (p_dir, p_cfg) in projects {
            ...
            if p_cfg.as_ref().is_some_and(|c| c.development) {
...
pub fn compile_file(file: &Path, output: Option<&Path>) -> bool {
    let validation = match auwgent_compile::validate_file_for_compile(file) {
        Ok(v) => v,
        Err(e) => {
            errors::report_analysis_error(&e);
            return false;
        }
    };
...
pub fn generate_file(file: &Path, target: &str, output: Option<&Path>) -> bool {
    ...
    if matches!(target, "dart") {
        let ir_module_path = out_dir.join(format!("{}.agent.ir.dart", stem));
        let ir_module = auwgent_codegen::generate_dart_ir_module(&ir);
        std::fs::write(&ir_module_path, ir_module).unwrap();
        eprintln!(
            "\x1b[32m✓\x1b[0m {} → {} (+ .agent.ir.dart)",
            file.display(),
            out_path.display()
        );
    } else {
        std::fs::write(
            out_dir.join(format!("{}.agent.json", stem)),
            serde_json::to_string_pretty(&ir).unwrap(),
        )
        .unwrap();
        eprintln!(
            "\x1b[32m✓\x1b[0m {} → {} (+ .agent.json)",
            file.display(),
            out_path.display()
        );
    }
