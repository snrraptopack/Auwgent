...
export class TypedAuwgent<
    ...
    Tools = ToolRegistry<IR>
> {
    private native: AuwgentRuntime = null as any;
    private nativeReady: Promise<AuwgentRuntime>;
    private ir: IR;
    ...
    constructor(ir: IR, config: AuwgentConfig<IR>) {
        ...
        const native = createNativeRuntimeSync(irJson);
        this.native = native;
        this.nativeReady = native
            ? Promise.resolve(native)
            : createNativeRuntime(irJson).then((runtime) => {
                this.native = runtime;
                this.configureNative(runtime, config);
                return runtime;
            });

        if (native) {
            this.configureNative(native, config);
        }
    }

    private configureNative(native: AuwgentRuntime, config: AuwgentConfig<IR>): void {
        if (config.context) {
        ...
        if (apiKeys?.geminiApiKey) {
            native.setGeminiDriver(apiKeys.geminiApiKey);
        }

        if (apiKeys?.openaiApiKey) {
            native.setOpenaiDriver(apiKeys.openaiApiKey);
        }

      if (apiKeys?.groqApiKey) {
            native.setGroqDriver(apiKeys.groqApiKey);
        }

        // Register custom drivers from IR
        this.registerCustomDrivers(native, apiKeys);
    ...
    private getNativeSync(): AuwgentRuntime {
        if (!this.native) {
            throw new Error(
                'Auwgent WASM runtime is still loading. Await ready() before using synchronous runtime methods.'
            );
        }
        return this.native;
    }
    ...
    private registerCustomDrivers(native: AuwgentRuntime, apiKeys: Record<string, string | undefined>) {
        const collectFromEntry = (entry: any) => {
            if (entry.defaultConfig?.model?.type === 'custom') {
                ...
                if (id && url) {
                    ...
                    if (key) {
                        native.setCustomDriver(id, key, url);
                    }
                }
            }
    ...
    async embed(text: string): Promise<number[]> {
        try {
            ...
            return await native.embed(text);
        } catch (e) {
            await this.triggerError(e as Error);
            throw e;
        }
    ...
    async embedBatch(texts: string[]): Promise<number[][]> {
        try {
            const native = await this.ensureNative();
            return await native.embedBatch(texts);
        } catch (e) {
            await this.triggerError(e as Error);
            throw e;
        }
    ...
    getMetadata(): RunMetadata {
        return JSON.parse(this.getNativeSync().getMetadata());
    }
    ...
    exportSession(): SessionState {
        return JSON.parse(this.getNativeSync().exportSession()) as SessionState;
    }

    /** Import a previously exported session state. */
    importSession(session: SessionState): void {
        this.getNativeSync().importSession(JSON.stringify(session));
    }

    /** Clear the session (fresh conversation). */
    clearSession(): void {
        this.getNativeSync().clearSession();
    ...
    generatePrompt<T extends ExtractHelperNames<IR> | undefined = undefined>(
        ...
        return this.getNativeSync().generatePrompt(helperName as string);
    }

    /** Get tool names defined in the IR. */
    getToolNames(): ExtractToolNames<IR>[] {
        return this.getNativeSync().getToolNames() as ExtractToolNames<IR>[];
    }

    /** Get tool schemas (for introspection). */
    getToolSchemas(): IRToolDef[] {
        return JSON.parse(this.getNativeSync().getToolSchemas()) as IRToolDef[];
    }

    /** Access the raw native binding for advanced usage. */
  get raw(): AuwgentRuntime {
        return this.getNativeSync();
    }
}
