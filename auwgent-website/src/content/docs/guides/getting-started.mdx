...
Auwgent requires **Node.js 18 or later**. No other runtime dependencies are needed.
...
<Tabs>
  ...
  <TabItem label="Python">
    ```bash
    pip install snrraptopack-auwgent-sdk
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="Python">
    ...
    generated/
      main.agent.json         ← compiled IR, describes your agent's intents and structure
      main_agent_types.py     ← generated Python types, ready to import
    ```
  </TabItem>
</Tabs>

You will only ever work with `main.agent.types.ts`. The JSON file is an internal artifact — it is already embedded in the types file. You do not need to read or reference it directly.

---

## Wiring it up

Open `main.agent.types.ts` and look at what was generated. You will find two named exports at the bottom of the file that are all you need for now: `auwgent` and `AuwgentConfig`.

`AuwgentConfig` is a TypeScript type that describes exactly what your agent needs to run — including which API key fields are required for the model you declared. Your editor will surface these through intellisense; you do not need to look them up.
